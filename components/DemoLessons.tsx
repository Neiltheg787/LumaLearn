      ...
      <svg viewBox="0 0 760 560" role="img" aria-label="Interactive beating heart blood flow">
        ...
        <g className="heart-transform">
          ...
          {Array.from({ length: 6 }).map((_, index) => (
            <circle key={`b-${index}`} r="7" className="blood blue">
              <animateMotion dur="2.8s" begin={`${index * 0.32}s`} repeatCount="indefinite"><mpath href="#bluePath" /></animateMotion>
            </circle>
          ))}
          {Array.from({ length: 6 }).map((_, index) => (
            <circle key={`r-${index}`} r="7" className="blood red">
              <animateMotion dur="2.8s" begin={`${index * 0.32}s`} repeatCount="indefinite"><mpath href="#redPath" /></animateMotion>
            </circle>
          ))}
        </g>
      </svg>
